...
{
    public GameObject NodeSpawn;
    public bool setConnections = false;
    [SerializeField]
    protected List<Node> m_Nodes = new List<Node>();

    public bool graphType2;
    ...
    private void CreateGraph()
    {
        ...
        if (graphType2 == false)
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                for (int j = 0; j < 100; j++)
                {
                    nodovi = Instantiate(NodeSpawn, new Vector3(i * 5, 0, j * 5), Quaternion.identity);
                    nodovi.transform.parent = gameObject.transform;

                    nodes.Add(nodovi.GetComponent<Node>());

                }
            }
        }
        else if (graphType2 == true)
        {
            /*
            GameObject nodovi;
            for (int i = 0; i < 25; i++)
